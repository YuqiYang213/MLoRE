...
class LoraBlock(nn.Module):
    ...
    def reparameter(self):
        self.virt_conv = nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, stride=1, padding=self.kernel_size//2)
        self.virt_conv.weight.data = (self.W.weight.data.permute(2, 3, 1, 0) @ self.M.weight.data.permute(2, 3, 1, 0)).permute(3, 2, 0, 1)
        self.virt_conv.bias.data = self.M.bias.data + (self.W.bias.data.reshape(1, 1, 1, -1) @ self.M.weight.data.permute(2, 3, 1, 0)).reshape(-1)
        self.reparam = True
    
    def forward(self, x):
        if self.reparam:
            out_1 = self.M(self.W(x))
            return out_1
        x = self.W(x)
        x = self.M(x)
        return x
...
def transI_fusebn(weight, bias, bn):
    gamma = bn.weight
    std = (bn.running_var + bn.eps).sqrt()
    weight = weight * ((gamma / std).reshape(-1, 1, 1, 1))
    bias = bn.bias + (bias - bn.running_mean) * gamma / std
    return weight, bias
...
class MOEBlock(nn.Module):
    ...
    def forward(self, x, task, route_feat_in=None):
        ...
        if not self.reparam:
            ...
            for i_s in range(n):
                ...
                virt_conv_weight = 0
                for i in range(self.num_lora):
                    virt_conv_weight = virt_conv_weight + self.lora_list_1[i].virt_conv.weight.data * route_1[i_s, i].reshape(-1)
                    virt_conv_bias = virt_conv_bias + self.lora_list_1[i].virt_conv.bias.data * route_1[i_s, i].reshape(-1)
                virt_conv_weight, virt_conv_bias = transI_fusebn(virt_conv_weight, virt_conv_bias, self.bn_all[task])
                virt_conv_weight = virt_conv_weight + self.conv2[task].virt_conv.weight.data * prob_mix[i_s, 0].reshape(-1)
                virt_conv_bias = virt_conv_bias + self.conv2[task].virt_conv.bias.data * prob_mix[i_s, 0].reshape(-1)
                virt_conv_weight = virt_conv_weight + self.share_conv.weight.data
                virt_conv_bias = virt_conv_bias + self.share_conv.bias.data
                out_list.append(F.conv2d(out[i_s].unsqueeze(0), virt_conv_weight, virt_conv_bias
                                    , stride=(1, 1), padding=(virt_conv_weight.shape[-1]//2, virt_conv_weight.shape[-1]//2)))
    ...
    def reparameter(self):
        for i in range(self.num_lora):
            self.lora_list_1[i].reparameter()
        for task in self.p.TASKS.NAMES:
            self.conv2[task].reparameter()
        self.reparam = True
...
class MLoRE(nn.Module):
    ...
    def reparameter(self):
        for i_layer in range(self.num_layers):
            self.MLoRE_1[i_layer].reparameter()
            self.MLoRE_2[i_layer].reparameter()
